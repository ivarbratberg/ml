...
train_columns = list(set(w5.columns)-set(['shop_name', 'item_name', 'item_category_name']))

for block_nr in range(25,last_block):
    idx = (w5['date_block_num']<block_nr) & (w5['date_block_num']>(block_nr-lag))
    X_train = w5[train_columns].loc[idx]
    Y_train = w5['item_cnt_next_month'].loc[idx  ]
    X_test = w5[train_columns].loc[w5['date_block_num']==block_nr]
    Y_test = w5['item_cnt_next_month'].loc[w5['date_block_num']==block_nr]
    ...
    if use_linear_meta:
        idx = (w5['date_block_num']<(block_nr-1)) & (w5['date_block_num']>(block_nr-lag))
        X_train_m = w5[train_columns].loc[idx]
        Y_train_m = w5['item_cnt_next_month'].loc[idx]
        X_test_m = w5[train_columns].loc[w5['date_block_num']==(block_nr-1)]
        Y_test_m = w5['item_cnt_next_month'].loc[w5['date_block_num']==(block_nr-1) ]
    ...
    X_test['current'] = w5['item_cnt_month'].loc[w5['date_block_num']== block_nr]
